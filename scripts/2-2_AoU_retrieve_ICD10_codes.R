## ICD10 codes from PhecodeX tables
...
system2("gsutil", args = c("cp", "updated_phecodex_map.csv", paste0(WORKSPACE_BUCKET, "/aou_gwas/phecodeX_files/")),
...
phecode$schema <- reticulate::r_to_py(list(bigquery$SchemaField("phecode", "STRING"),bigquery$SchemaField("ICD",  "STRING"),
...
peek <- client$query("SELECT vocabulary_id, ICD, phecode FROM phecodex LIMIT 5",job_config = job_config)$to_dataframe()
...
ehr_qry <- client$query("SELECT DISTINCT person_id FROM cb_search_person WHERE has_ehr_data = 1",job_config = job_config2)
...
#write out 
write_csv(phe_bool, "mcc2_phecodex_table_v8.csv")